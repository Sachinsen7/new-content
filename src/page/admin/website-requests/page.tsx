...
import { Eye, MessageSquare, Plus, Search, Filter, Download } from "lucide-react"

export default function WebsiteRequestsPage() {
  const [submissions, setSubmissions] = useState<Submission[]>(
    mockSubmissions.filter((sub) => sub.type === "website-building"),
  )
  ...
  const exportToCSV = () => {
    const headers = ["Client Name", "Email", "Company", "Package", "Status", "Created Date"]
    const csvContent = [
      headers.join(","),
    ...
    a.href = url
    a.download = "website-requests.csv"
    a.click()
      ...
      <AdminLayout>
        <div className="p-4 sm:p-6 lg:p-8">
          <div className="flex justify-between items-center mb-8">
            <div>
              <h1 className="text-3xl font-bold mb-2 text-[#eceafa]">Website Requests</h1>
              <p className="text-[#88adce]">Manage website building service requests</p>
            </div>
            <Button onClick={exportToCSV} className="bg-[#88adce] text-[#000000] hover:bg-[#4d93cb]">
          ...
          <Card className="mb-6 bg-[#000000] border-[#88adce]/20">
            <CardHeader>
              <CardTitle className="text-lg text-[#eceafa]">Filters & Search</CardTitle>
            </CardHeader>
            <CardContent>
                ...
                <div>
                  ...
                  <Select value={statusFilter} onValueChange={setStatusFilter}>
                    ...
                    <SelectContent className="bg-[#000000] border-[#88adce]/30">
                      <SelectItem value="all" className="text-[#eceafa] hover:bg-[#88adce]/10">
                        All Statuses
                      </SelectItem>
                      <SelectItem value="new" className="text-[#eceafa] hover:bg-[#88adce]/10">
                        New
                      </SelectItem>
                      <SelectItem value="in-progress" className="text-[#eceafa] hover:bg-[#88adce]/10">
                        In Progress
                      </SelectItem>
                      <SelectItem value="completed" className="text-[#eceafa] hover:bg-[#88adce]/10">
                        Completed
                      </SelectItem>
                      <SelectItem value="cancelled" className="text-[#eceafa] hover:bg-[#88adce]/10">
                        Cancelled
                      </SelectItem>
                    </SelectContent>
                  </Select>
          ...
          <Card className="bg-[#000000] border-[#88adce]/20">
            <CardHeader>
              <CardTitle className="text-[#eceafa]">Website Building Requests ({filteredSubmissions.length})</CardTitle>
              <CardDescription className="text-[#88adce]">
                Complete list of website building service requests
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                ...
                <TableBody>
                  {filteredSubmissions.map((submission) => (
                    <TableRow key={submission.id} className="border-[#88adce]/20">
                      <TableCell>
                        <div>
                          <p className="font-medium text-[#eceafa]">{submission.clientName}</p>
                          <p className="text-sm text-[#88adce]">{submission.clientEmail}</p>
                          {submission.company && <p className="text-sm text-[#88adce]">{submission.company}</p>}
                        </div>
                      </TableCell>
                      <TableCell className="capitalize text-[#eceafa]">{submission.package}</TableCell>
                      <TableCell>
                        <Select
                        ...
                        <div className="flex space-x-2">
                          <Dialog>
                            ...
                            <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto bg-[#000000] border-[#88adce]/30 text-[#eceafa]">
                              <DialogHeader>
                                <DialogTitle className="text-[#eceafa]">Website Request Details</DialogTitle>
                                <DialogDescription className="text-[#88adce]">
                                  Website building request from {submission.clientName}
                                </DialogDescription>
                              </DialogHeader>
                              ...
                              {selectedSubmission && (
                                <div className="space-y-6">
                                  {/* Client Info */}
                                  <div>
                                    <h3 className="text-lg font-semibold mb-3 text-[#eceafa]">Client Information</h3>
                                    <div className="grid grid-cols-2 gap-4">
                                      <div>
                                        <Label className="text-[#88adce]">Name</Label>
                                        <p className="text-sm text-[#eceafa]">{selectedSubmission.clientName}</p>
                                      </div>
                                      <div>
                                        <Label className="text-[#88adce]">Email</Label>
                                        <p className="text-sm text-[#eceafa]">{selectedSubmission.clientEmail}</p>
                                      </div>
                                      {selectedSubmission.clientPhone && (
                                        <div>
                                          <Label className="text-[#88adce]">Phone</Label>
                                          <p className="text-sm text-[#eceafa]">{selectedSubmission.clientPhone}</p>
                                        </div>
                                      )}
                                      {selectedSubmission.company && (
                                        <div>
                                          <Label className="text-[#88adce]">Company</Label>
                                          <p className="text-sm text-[#eceafa]">{selectedSubmission.company}</p>
                                        </div>
                                      )}
                                    </div>
                                  </div>
                                  ...
                                  {/* Form Data */}
                                  <div>
                                    <h3 className="text-lg font-semibold mb-3 text-[#eceafa]">Project Details</h3>
                                  ...
                                  <div>
                                    ...
                                    <div className="space-y-3">
                                      {selectedSubmission.notes.map((note) => (
                                        <div key={note.id} className="border border-[#88adce]/20 rounded-lg p-3">
                                          <p className="text-sm text-[#eceafa]">{note.content}</p>
                                    ...
                                    {/* Add Note */}
                                    <div className="mt-4">
                                      <Label htmlFor="new-note" className="text-[#88adce]">
                                        Add Note
                                      </Label>
                                      <Textarea
                                        id="new-note"
                                      ...
                                      />
                                      <Button
                                        onClick={() => addNote(selectedSubmission.id)}
